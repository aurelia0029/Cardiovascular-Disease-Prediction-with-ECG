"""
Sequence Length Experiment - Analyze model performance vs sequence length.

This script trains CNN-LSTM models with different sequence lengths
to analyze the optimal number of consecutive beats for prediction.
"""
...
def train_single_model(X_train, y_train, X_val, y_val, device, num_epochs=40, patience=5):
...
def run_sequence_experiment(args):
    """Run sequence length experiment with configurable parameters."""

    print(f"Sequence Length Experiment")
    print(f"{'='*60}")
    print(f"Data directory: {args.data_dir}")
    print(f"Prediction window (w_before): {args.w_before}")
    print(f"Sequence range: {args.min_seq} to {args.max_seq} (step: {args.step})")
    ...
    # Generate sequence length values
    seq_len_list = list(range(args.min_seq, args.max_seq + 1, args.step))
    results = []
    ...
    # Experiment loop
    for seq_len in tqdm(seq_len_list, desc="Sequence experiments"):
        print(f"\n{'='*60}")
        print(f"Sequence length = {seq_len} beats")
        print(f"{'='*60}")

        # Create sequences
        X_seq, y_seq = create_sequences_with_offset(
            beat_windows, beat_labels, seq_len, args.w_before
        )
    ...
    # Save results
    output_file = f"sequence_results_w{args.w_before}.json"
...
def main():
    parser = argparse.ArgumentParser(
        description='Sequence Length Experiment - Analyze optimal sequence length'
    )
    ...
    parser.add_argument('--w_before', type=int, default=1,
                        help='Prediction window (beats ahead)')
    parser.add_argument('--min_seq', type=int, default=1,
                        help='Minimum sequence length')
    parser.add_argument('--max_seq', type=int, default=11,
                        help='Maximum sequence length')
    parser.add_argument('--step', type=int, default=1,
                        help='Step size for sequence length values')
    ...
    # Run experiment
    results = run_sequence_experiment(args)
